...
let totalOfCollections = 2;
let countOfCompletedCollections = 0;
...
function writeResultToFile() {
    if (countOfCompletedCollections === totalOfCollections){
        fs.writeFile('data.json', JSON.stringify(resultsForFirestore, undefined, 2), {encoding: 'utf8', flag: 'w'}, function (err) {
            if (err) throw err;
            console.log('Exported collection count:' + countOfCompletedCollections);
        });
    }
}